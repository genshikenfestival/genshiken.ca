...
export default function Button({
  ...
  return href ? (
    ...
    <HeadlessButton
      className={`bg-brand-purple center group rounded-2xl px-16 py-4 text-lg text-white transition-all duration-200 data-active:bg-gray-500 data-hover:cursor-pointer data-hover:bg-gray-500 ${className ?? ""}`}
    >
      {children}
      {showIcon ? (
        <i className={`bi bi-${showIcon} ml-2 translate-y-5`}></i>
